...
class EntailmentAF(BaseModel):
    explicit: list[str]
    implicit: list[str]

class MetEntailmentAF(BaseModel):
    implicit: list[str]

class MetFramEntailmentAF(BaseModel):
    problem_def: str
    cause: str
    moral_eval: str
    treatment_rec: str
...
class EntailmentPromptEngineer:
    ...
    def get_af(self):
        ...
        if self.prompt_file == "generate_entailments.json":
            return EntailmentAF

        elif self.prompt_file == "generate_metaphor_entailments.json":
            return MetEntailmentAF
        
        elif self.prompt_file == "generate_metaphor_framing_entailments.json":
            return MetFramEntailmentAF
    ...
    def get_data(self) -> dict:
        ...
        metaphor_df = self.data_loader.load_metaphor_classification_results(metaphors_only=True).set_index("id")
        ...
        self.metaphor_docs = {
            k: {"text": f"Metaphorical verb: {v['source_word']} \nSentence: {v['sentence']}"} 
...
def main(args, env_vars, LOGGER):
    ...
    prompt_engineer = EntailmentPromptEngineer(config, data_loader, LOGGER, shuffle_docs=args.shuffle_docs)
